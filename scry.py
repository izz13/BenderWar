...
scrybkrd = pygame.image.load("spclevt.png")
...
for i in range(tn):
    hitObjects["turrets"].append(turret.Turret(randint(0, 2800), randint(0, 2240), randint(1, 120), "zoombie.png", "zmbe", b_size, pimage="hand.png"))
hitObjects["turrets"].append(turret.Necromancer(randint(1400, 1400), randint(1120, 1120), randint(1, 120), "ncrmncr.png", "ncrmncr", b_size))
isRunning = True
while isRunning:
    tick = clock.get_time()
    ...
    bkrdpos[0] += vlx
    bkrdpos[1] += vly
    screen.blit(scrybkrd, bkrdpos)
    for t in hitObjects["turrets"]:
        if bkrdpos[0] > 0:
            bkrdpos[0] = 0
            vlx = 0
        if bkrdpos[1] > 0:
        ...
        if bkrdpos[1] < -1920:
            bkrdpos[1] = -1920
            vly = 0
        t.update(screen, player.x, player.y, tick, player, vlx, vly)
        if t.destroyed:
            hitObjects["turrets"].remove(t)
            tn -= 1
        if type(t) == turret.Necromancer:
            if t.spawn == True:
                hitObjects["turrets"].append(t.spawnskeleton())
                t.spawn = False
