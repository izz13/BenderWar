...
class ScaryWorld:
    def __init__(self, screen, fps, clock):
        ...
        self.scrybkrd = pygame.image.load("spclevt.png")
    ...
    def gameloop(self):
        ...
        for t in self.hitObjects["turrets"]:
            ...
            if self.bkrdpos[1] < -1920:
                self.bkrdpos[1] = -1920
                vly = 0
            t.update(self.screen, self.player.x, self.player.y, tick, self.player, vlx, vly)
            if t.destroyed:
                self.hitObjects["turrets"].remove(t)
                self.tn -= 1
            if type(t) == turret.Necromancer:
                if t.spawn == True:
                    self.hitObjects["turrets"].append(t.spawnskeleton())
                    t.spawn = False
